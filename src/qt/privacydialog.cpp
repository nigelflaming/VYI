...
#include "zvyigratcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zVYI ought to be enough for anybody." - Bill Gates, 2017
    ui->zVYIpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzVYISyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzVYI->setEnabled(false);
        ui->pushButtonMintzVYI->setToolTip(tr("zVYI is currently disabled due to maintenance."));

        ui->pushButtonSpendzVYI->setEnabled(false);
        ui->pushButtonSpendzVYI->setToolTip(tr("zVYI is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zVYIpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzVYI_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVYI is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zVYI...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zVYI in ") +
...
void PrivacyDialog::on_pushButtonSpendzVYI_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVYI is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zVYI
        sendzVYI();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zVYI
    sendzVYI();
}

void PrivacyDialog::on_pushButtonZVyigratControl_clicked()
{
    ZVyigratControlDialog* zVYIControl = new ZVyigratControlDialog(this);
    zVYIControl->setModel(walletModel);
    zVYIControl->exec();
}

void PrivacyDialog::setZVyigratControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzVYISelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVYI()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Vyigrat Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zVYIpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zVYI is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " VYIGRAT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zVYI</b>";
    ...
    // use mints from zVYI selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZVyigratControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZVyigratControlDialog::GetSelectedMints();
    }
    ...
    // Clear zvyigrat selector in case it was used
    ZVyigratControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zVYI Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Vyigrat, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zVYI Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zVYI </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zVYI "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zVYI "));
    ui->labelzVYIAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzVYISyncStatus->setVisible(fShow);
}
