...
#include "ui_zvyigratcontroldialog.h"
...
std::list<std::string> ZVyigratControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZVyigratControlDialog::listMints;

ZVyigratControlDialog::ZVyigratControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZVyigratControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZVyigratControlDialog::~ZVyigratControlDialog()
{
    delete ui;
}

void ZVyigratControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVyigratControlDialog::updateList()
{
...
void ZVyigratControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVyigratControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZVyigrat_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVyigratControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZVyigratControlDialog::GetSelectedMints()
{
...
void ZVyigratControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
